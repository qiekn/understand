  ...
  for i = 1, len(FloodFillRes[currentPanel]) do
    for j = 1, len(FloodFillRes[currentPanel][i]) do
      ...
      for k = 1, len(FloodFillRes[currentPanel][i]) do
        if
          (FloodFillRes[currentPanel][i][j].x - FloodFillRes[currentPanel][i][k].x) <= 1
          and (FloodFillRes[currentPanel][i][j].x - FloodFillRes[currentPanel][i][k].x) >= 0
          and (FloodFillRes[currentPanel][i][j].y - FloodFillRes[currentPanel][i][k].y) <= 1
          and (FloodFillRes[currentPanel][i][j].y - FloodFillRes[currentPanel][i][k].y) >= 0
        then
          tt = tt + 1
        end
      end
