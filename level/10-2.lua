  ...
  for i = 1, len(FloodFillRes[currentPanel]) do
    ...
    for j = 1, len(FloodFillRes[currentPanel][i]) do
      if map[currentPanel][FloodFillRes[currentPanel][i][j].x][FloodFillRes[currentPanel][i][j].y].str == "~" then
        if
          map[currentPanel][FloodFillRes[currentPanel][i][j].x][FloodFillRes[currentPanel][i][j].y].num
          ~= len(FloodFillRes[currentPanel][i])
        then
          ans2 = false
        end
      end
    ...
    ans3 = ans3
      and AreaMirror(FloodFillRes[currentPanel][i], FloodFillRes[currentPanel][i], maxX + minX, maxY + minY, "reverse")
  end
