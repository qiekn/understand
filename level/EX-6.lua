...
function newlevel.SpecialDraw()
  ...
  if(len(visit[panel_cnt+1-currentPanel])>0)then
    x0,y0=toScreen(mapW[currentPanel]+1-visit[panel_cnt+1-currentPanel][1].x,visit[panel_cnt+1-currentPanel][1].y)
    drawSquare(x0,y0,boxwidth*0.8)
    if(len(visit[panel_cnt+1-currentPanel])>=2)then
      x1,y1=toScreen(mapW[currentPanel]+1-visit[panel_cnt+1-currentPanel][2].x,visit[panel_cnt+1-currentPanel][2].y)
      pt={x0+(x1-x0)*0.4,y0+(y1-y0)*0.4,x1,y1}
      for i=3,len(visit[panel_cnt+1-currentPanel])do
        x1,y1=toScreen(mapW[currentPanel]+1-visit[panel_cnt+1-currentPanel][i].x,visit[panel_cnt+1-currentPanel][i].y)
        table.insert(pt,x1)
        table.insert(pt,y1)
      end
      --rPrint(pt)
