...
--  drawTextLeft(300,400,"abcdefghijklmnopqrstuvwxyz")
    ...
    for j=1,mapHeight do
      ...
      if newlevel0.HideShape~=true then
        for k=1,len(map[currentPanel][i][j].str)do
          ...
          elseif(ch=="=")then
            drawSmallLine(boxX,boxY,size,newlevel0.curve[map[currentPanel][i][j].char],visit_grid[currentPanel][i][j])
          elseif(ch=="$")then
  ...
  if(len(visit[currentPanel])>0)then
    ...
    if(len(visit[currentPanel])>=2)then
      ...
      if dis(visit[currentPanel][1].x,visit[currentPanel][1].y,visit[currentPanel][2].x,visit[currentPanel][2].y)~=1 then
        pt={x1,y1}
      end
      for i=3,len(visit[currentPanel])do
        x1,y1=toScreen(visit[currentPanel][i].x,visit[currentPanel][i].y)
        if dis(visit[currentPanel][i].x,visit[currentPanel][i].y,visit[currentPanel][i-1].x,visit[currentPanel][i-1].y)~=1 then
          if len(pt)>=4 then
    ...
    for i=1,correct_num do
      ...
      if newlevel0.CircleColor~=nil then
        ...
        elseif type(newlevel0.CircleColor[i])=="table"then
          ...
          love.graphics.arc("fill",correct_x+(i-1)*uiSize,correct_y,uiSize*0.25+math.min(screenWidth/120,screenHeight/80)/2, math.pi/2, math.pi*3/2 )
          love.graphics.setColor(frontcolors[newlevel0.CircleColor[i][2]])
          love.graphics.arc("fill",correct_x+(i-1)*uiSize,correct_y,uiSize*0.25+math.min(screenWidth/120,screenHeight/80)/2, -math.pi/2, math.pi/2 )
          if correct[currentPanel][i]==false then
            love.graphics.setColor(backcolor)
            love.graphics.circle("fill",correct_x+(i-1)*uiSize,correct_y,uiSize*0.25-math.min(screenWidth/120,screenHeight/80)/2)
            love.graphics.setColor(frontcolors[newlevel0.CircleColor[i][1]])
            local pt={}
            local mm=((math.min(screenWidth/120,screenHeight/80)-2)/uiSize)/2
            for t=0,100 do
              table.insert(pt,correct_x+(i-1)*uiSize-uiSize*0.25*math.sin(mm/2+(math.pi-mm)*t/100))
...
function level.update()
  ...
  if(love.timer.getTime()-showhint<=0.5)and(x>screenWidth-uiSize*2.2)and(x<screenWidth-uiSize*1.4)and(y<=uiSize*1.2)then
...
function level.mousepressed(x,y,button)
  if(gamestate=="ready")then
    if(button==1)then
      ...
      if(x<uiSize*1.2)and(y>=screenHeight/2-uiSize)and(y<=screenHeight/2+uiSize)and(currentPanel>1)then
        currentPanel=currentPanel-1
        mapWidth=mapW[currentPanel]
        mapHeight=mapH[currentPanel]
        last_panel[currentLevel]=currentPanel
      elseif(x>screenWidth-uiSize*1.2)and(y>=screenHeight/2-uiSize)and(y<=screenHeight/2+uiSize)and(currentPanel<=panel_cnt)then
        currentPanel=currentPanel+1
        if(currentPanel>panel_cnt)then
          if(AllVictory())then
...
--    end
  elseif(gamestate=="draw")then
    ...
    --  love.mouse.setCursor()
      ...
      level_lines[currentLevel][currentPanel]={}
      for i=1,len(visit[currentPanel])do
        table.insert(level_lines[currentLevel][currentPanel],{x=visit[currentPanel][i].x,y=visit[currentPanel][i].y})
      end
      save()
      if mute~=2 then
        if(AllVictory())then
