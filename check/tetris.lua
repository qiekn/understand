      ...
      for t=1,len(shapes[i])do
        if isValid(grid,grid_bottomleft,shapes[i][t])then
          for j=1,len(shapes[i][t])do
            grid[grid_bottomleft.x-shapes[i][t].bottomleft.x+shapes[i][t][j].x][grid_bottomleft.y-shapes[i][t].bottomleft.y+shapes[i][t][j].y]=false
          end
          if try_tetris(grid,shapes,nn+1) then
            return true
          end
          for j=1,len(shapes[i][t])do
            grid[grid_bottomleft.x-shapes[i][t].bottomleft.x+shapes[i][t][j].x][grid_bottomleft.y-shapes[i][t].bottomleft.y+shapes[i][t][j].y]=true
          end
        end
      end
      shapes[i].able=true
...
function isValid(grid,grid_bottomleft,shape)
  for j=1,len(shape)do
    ...
    if grid[grid_bottomleft.x-shape.bottomleft.x+shape[j].x][grid_bottomleft.y-shape.bottomleft.y+shape[j].y]~=true then
      return false
    end
  end
  return true
